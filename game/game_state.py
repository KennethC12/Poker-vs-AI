...
class ChatLog:
    ...
    def add_message(self, message):
        ...
        if len(self.messages) > self.max_messages:
            ...
            self.scroll_offset = 0  # No need to scroll if there aren't enough messages yet
    ...
    def handle_scroll(self, direction):
        ...
        elif direction < 0 and self.scroll_offset < len(self.messages) - self.max_messages:
    ...
    def draw(self, screen):
        ...
        visible_messages = self.messages[self.scroll_offset:self.scroll_offset + self.max_messages]
        ...
        for message in visible_messages:
            ...
            screen.blit(text_surface, (x_position, y_offset))  # Draw message at (x_position, y_offset)
...
class GameState:
    ...
    def handle_bet(self, player, amount, chat_log):
        ...
        if amount > player.chips:
            raise ValueError(f"{player.name} does not have enough chips to bet {amount}.")
        ...
        chat_log.add_message(f"{player.name} has bet {amount} chips. Pot is now {self.pot} chips.")
    ...
    def handle_fold(self, player, chat_log):
        ...
        try:
            ...
            if self.only_one_player_left():
                ...
                chat_log.add_message(f"{winner.name} wins the pot of {self.pot} chips by default!")
                self.distribute_pot_to_winner(winner)

                pygame.time.delay(2000)  # Short delay before resetting for the new round
    ...
    def next_player(self, deck, chat_log):
        ...
        if self.all_players_have_acted():
            ...
            chat_log.add_message(f"Next player is {self.players[self.current_player_index].name}")
    ...
    def post_blinds(self):
        ...
        big_blind_player = self.players[(self.small_blind_index + 1) % len(self.players)]
        ...
        print(f"{small_blind_player.name}'s current bet: {small_blind_player.current_bet}")
    ...
    def advance_stage(self, deck, chat_log):
        ...
        chat_log.add_message(f"Advancing stage: {self.stage}, Current bet: {self.current_bet}")
        ...
        if self.stage == PRE_FLOP:
            ...
            self.current_bet = 0  
            print(f"Game's current bet reset to {self.current_bet} for the new stage: {self.stage}")
            chat_log.add_message(f"Game's current bet reset to {self.current_bet} for the new stage: {self.stage}")
            ...
            chat_log.add_message("All community cards have been dealt. Determining the winner...")
            ...
            if isinstance(winners, list) and len(winners) > 1:
                ...
                chat_log.add_message(f"{winners.name} wins the pot of {self.pot} chips!")
        ...
        if self.stage != PRE_FLOP:  # Only reset bets after the pre-flop stage
            for player in self.players:
                ...
                chat_log.add_message(f"{player.name}'s bet reset to {player.current_bet}.")
    ...
    def evaluate_hands(self):
        player_hands = {player: player.hand + self.community_cards for player in self.players}
        evaluated_hands = {player: evaluate_hand(hand) for player, hand in player_hands.items()}
...
def handle_player_action(action, deck):
    ...
    print(f"Before action: {current_player.name}'s current bet: {current_player.current_bet}, Game's current bet: {game_state.current_bet}")
    ...
    print(f"After action: {current_player.name}'s current bet: {current_player.current_bet}, Game's current bet: {game_state.current_bet}")
...
def assign_blinds(chat_log):
    ...
    big_blind_player = game_state.players[(game_state.current_player_index + 1) % len(game_state.players)]
    ...
    chat_log.add_message(f"{small_blind_player.name} posts the small blind of {SMALL_BLIND} chips.")
    chat_log.add_message(f"{big_blind_player.name} posts the big blind of {BIG_BLIND} chips.")
